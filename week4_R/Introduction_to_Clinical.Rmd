...
colnames(mtcars)
rownames(mtcars)
mtcars$mpg # fill in name of carburetor column here
```
...
```{r Exercise 1.11}
am_col <- which(colnames(mtcars) == "am")
...
# Extract patient clinical data, removing the first two rows
...
```{r}
write.csv(clinic, "/home1/fkabbani/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/fkabbani/analysis_data/brca_clinical_data.csv")
```
...
death_days_to_mask <- ifelse(clinic$death_days_to == "[Not Applicable]", FALSE, TRUE)

cleanedClinic <- clinic[death_days_to_mask, ]

plot(cleanedClinic$age_at_diagnosis, cleanedClinic$death_days_to,
     main = "Age at Diagnosis vs days to death",
     xlab = "Age",
     ylab = "Days",
     col = "blue",
     pch = 19)
...
cleanedClinic$age_group <- ifelse(cleanedClinic$age_at_diagnosis > 50, "Old", "Young")

boxplot(cleanedClinic$death_days_to ~ cleanedClinic$age_group, 
        main = "Age at Diagnosis vs Days to Death",
        xlab = "Age Group",
        ylab = "Days to Death",
        notch = TRUE,
...
```{r}
male_mask <- cleaned_clinic$gender == "Male"

```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under35_over60_mask <- cleaned_clinic$age_at_diagnosis < 35 |cleaned_clinic$age_at_diagnosis > 60
...
```{r}
no_middle_clinic <- cleaned_clinic[under35_over60_mask, ]
...
```{r}
# Create age_category based on age_at_diagnosis
...
cleaned_clinic$survival_time <- ifelse(
  cleaned_clinic$vital_status == "Alive",
  cleaned_clinic$last_contact_days_to,
  cleaned_clinic$death_days_to
)

cleaned_clinic$survival_time[cleaned_clinic$death_days_to == "[Not Applicable]"] <- NA
...
The survival rate/ patients still alive

# 2. What does the x axis represent?
The time in days since diagnosis
...
p= 0.77 which means there is no significantly different difference between survival probability and time
